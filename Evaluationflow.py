########################################################################################## 
####    Code to evaluate a CNN for semantic image pixel labelling of OCT images
...
from evaluate_model_oct_functions import get_model_name, load_image, label_on_fly, make_image, open_file 
from evaluate_model_oct_functions import apply_em, save_image, create_f1_score, get_kernel_size, get_label_from_cnn
...
with open(cwd_data+'F1_scores%scnn.txt'%str(box_size), 'w') as f_cnn:
    f_cnn.write('F1 scores for box_size %s \n' % str(box_size))
    f_cnn.write('Image, f1score 0, f1score 1, f1score 2, f1score 3, f1score 4, f1score 5\n')
    
    with open(cwd_data+'F1_scores%scnn_em.txt'%str(box_size), 'w') as f_cnn_em:
        f_cnn_em.write('F1 scores for box_size %s \n' % str(box_size))
        f_cnn_em.write('Image, f1score 0, f1score 1, f1score 2, f1score 3, f1score 4, f1score 5\n')
        ...
        for data_name in data_eval:
            ...
            size = im.size
            print('####Succesfully loaded image %s' % data_name)
            ...
            for i in xrange(int((height-max_box_size)/stride)):
                ...
                predictions_temp = []
                labels_temp = []
                #Go through the width (x-axes) of the image using the same centerpoint
